...
func (r *chainRepo) CreateChain(ctx context.Context, chain *domain.Chain) error {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO chains (id, account_id, underlying_symbol, original_trade_id, created_at, closed_at, strategy_type, attribution_gap)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		s.ID, s.AccountID, s.UnderlyingSymbol, s.OriginalTradeID, s.CreatedAt, s.ClosedAt, s.StrategyType, s.AttributionGap,
	)
	if err != nil {
...
func (r *chainRepo) GetChainByTradeID(ctx context.Context, tradeID string) (*domain.Chain, error) {
	...
	err := r.db.QueryRowContext(ctx,
		...
		`SELECT id, account_id, underlying_symbol, original_trade_id, created_at, closed_at, strategy_type, attribution_gap
		 ...
		 UNION
		 SELECT c.id, c.account_id, c.underlying_symbol, c.original_trade_id, c.created_at, c.closed_at, c.strategy_type, c.attribution_gap
		 FROM chains c
...
func (r *chainRepo) ListChainsByAccount(ctx context.Context, accountID string, openOnly bool) ([]domain.Chain, error) {
	query := `SELECT id, account_id, underlying_symbol, original_trade_id, created_at, closed_at, strategy_type, attribution_gap
	...
	for rows.Next() {
		var s model.Chain
		if err := rows.Scan(&s.ID, &s.AccountID, &s.UnderlyingSymbol, &s.OriginalTradeID, &s.CreatedAt, &s.ClosedAt, &s.StrategyType, &s.AttributionGap); err != nil {
